...
class TradingBot:
            ...
            for symbol in sorted_pairs:
                try:
                    ...
                    if state in ['UPTREND', 'DOWNTREND', 'RANGE']:
                        ...
                        kb.append([InlineKeyboardButton(side_label, callback_data=f"trade:{symbol}:{side_btn}")])
    ...
    def run(self):
        ...
        application.add_handler(CommandHandler("dash", self.dash_command))
        application.add_handler(CommandHandler("health", self.health_command))
        application.add_handler(CommandHandler("selftest", self.selftest_command))
        ...
        application.add_handler(CommandHandler("runtime", self.where_command))
        application.add_handler(CommandHandler("version", self.where_command))
        application.add_handler(CommandHandler("token", self.token_command))
        application.add_handler(CommandHandler("auto_on", self.cmd_auto_on))
        application.add_handler(CommandHandler("auto_off", self.cmd_auto_off))
        application.add_handler(CommandHandler("hybrid", self.hybrid_command))
    ...
    async def selftest_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # 7. JobQueue (informational only; do not fail selftest)
        try:
    ...
    def run(self):
        ...
        # Add generic callback handler (must be last to catch all other callbacks)
        if hasattr(self, 'handle_callback'):
             self.application.add_handler(CallbackQueryHandler(self.handle_callback))
        
        # Trade Wizard Conversation Handler
        wiz_trade_handler = ConversationHandler(
            entry_points=[
                MessageHandler(filters.Regex(r"^🚀 معامله جدید$"), self.wiz_start),
                CommandHandler("wiz", self.wiz_start),
