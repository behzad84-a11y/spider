...
class ScannerStateRegistry:
    ...
    def get_field(self, name: str, key: str, default: Any = None) -> Any:
        """Get a specific field from a scanner's state."""
        state = self.get(name)
...
class DashboardManager:
    ...
    def _scanner_summary(self) -> str:
        ...
        for name, state in registry.get_all().items():
            ...
            if name == 'AI_Optimizer':
                ...
                running_status = state.get('running_status', 'IDLE')
                
                # Determine session state based on candle count
                session_state = 'TRACKING'
                if candle == 0:
                    session_state = 'PREOPEN'
                elif candle >= 18 and is_locked:
                    session_state = 'LOCKED'
                elif candle >= 18:
                    session_state = 'POST_QWINDOW'
                
                # Show IDLE state if not started yet
                if running_status == 'IDLE' and candle == 0:
                    lines.append(f"  {icon} <b>QGLN:</b> IDLE (not started)")
                elif session_state == 'PREOPEN':
                    lines.append(f"  {icon} <b>QGLN:</b> PREOPEN (waiting for 09:30 NY)")
                elif session_state == 'LOCKED' or (is_locked and candle >= 18):
                    lines.append(f"  {icon} <b>QGLN:</b> LOCKED | Candle {candle}/18 ({q_prob}%) | Q-Channel: {q_high:.2f}/{q_low:.2f} | Gap: {gap_status}")
                elif session_state == 'POST_QWINDOW':
                    lines.append(f"  {icon} <b>QGLN:</b> POST-QWINDOW | Candle {candle}/18 | Q-Channel: {q_high:.2f}/{q_low:.2f} | Gap: {gap_status}")
                else:
                    lines.append(f"  {icon} <b>QGLN:</b> TRACKING | Candle {candle}/18 ({q_prob}%) | Q-Channel: {q_high:.2f}/{q_low:.2f} | Gap: {gap_status}")
            else:
                # Show explicit IDLE status if never run
                running_status = state.get('running_status', 'IDLE')
                if running_status == 'IDLE' and scans == 0 and last_run == 'Never':
                    lines.append(f"  {icon} <b>{name}:</b> IDLE (not started)")
                else:
    ...
    def get_unified_dashboard(self, full: bool = False) -> str:
        """
        Unified dashboard entry point for /dash and /dash full.
        Returns short (compact), verbose, or full report based on argument.
        """
        if full:
...
class DigestReporter:
    ...
    def __init__(self, bot_instance, interval_minutes=60, message_callback=None):
        ...
        self._quiet_start = 23  # 23:00
        self._quiet_end = 7     # 07:00 (for _is_quiet_hour fallback when no silent_manager)
    ...
    async def run_loop(self):
        ...
        while self._running:
            try:
                ...
                silent_mgr = getattr(self.bot, 'silent_manager', None)
                if silent_mgr and not silent_mgr.should_send(3):  # 3 = LOW priority (suppressed during silent)
                    continue
